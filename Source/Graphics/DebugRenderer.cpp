...
DebugRenderer::DebugRenderer(ID3D11Device* device)
{
	...
	// 四角形のメッシュ作成
	CreateRectangleMesh(device, 1.0f, 1.0f);
}
...
void DebugRenderer::Render(ID3D11DeviceContext* context, const DirectX::XMFLOAT4X4& view, const DirectX::XMFLOAT4X4& projection)
{
	...
	// 矩形描画
	context->IASetVertexBuffers(0, 1, rectangleVertexBuffer.GetAddressOf(), &stride, &offset);
	for (const Rectangle& rectangle : rectangles)
	{
		// ワールドビュープロジェクション行列作成
		DirectX::XMMATRIX S = DirectX::XMMatrixScaling(rectangle.width, rectangle.height, 1.0f);
		DirectX::XMMATRIX T = DirectX::XMMatrixTranslation(rectangle.center.x, rectangle.center.y, 0.0f);
		DirectX::XMMATRIX W = S * T;
		DirectX::XMMATRIX WVP = W * VP;

		// 定数バッファ更新
		CbMesh cbMesh;
		cbMesh.color = rectangle.color;
		DirectX::XMStoreFloat4x4(&cbMesh.wvp, WVP);

		context->UpdateSubresource(constantBuffer.Get(), 0, 0, &cbMesh, 0, 0);
		context->Draw(rectangleVertexCount, 0);
	}
	rectangles.clear();
}
...
// 矩形描画
void DebugRenderer::DrawRectangle(const DirectX::XMFLOAT2& center, float width, float height, const DirectX::XMFLOAT4& color)
{
	// 矩形を描画するためのデータをセットアップ
	Rectangle rectangle;
...
// 矩形メッシュ作成
void DebugRenderer::CreateRectangleMesh(ID3D11Device* device, float width, float height)
{
	rectangleVertexCount = 4; // 四角形は4つの頂点を持つ

	// 頂点データの作成
	std::unique_ptr<DirectX::XMFLOAT3[]> vertices = std::make_unique<DirectX::XMFLOAT3[]>(rectangleVertexCount);

	// 矩形の頂点座標を設定
	vertices[0] = DirectX::XMFLOAT3(-width / 2.0f, -height / 2.0f, 0.0f); // 左下
	vertices[1] = DirectX::XMFLOAT3(width / 2.0f, -height / 2.0f, 0.0f);  // 右下
	vertices[2] = DirectX::XMFLOAT3(width / 2.0f, height / 2.0f, 0.0f);   // 右上
	vertices[3] = DirectX::XMFLOAT3(-width / 2.0f, height / 2.0f, 0.0f);  // 左上

	// 頂点バッファの作成
	D3D11_BUFFER_DESC desc = {};
	D3D11_SUBRESOURCE_DATA subresourceData = {};

	desc.ByteWidth = static_cast<UINT>(sizeof(DirectX::XMFLOAT3) * rectangleVertexCount);
	desc.Usage = D3D11_USAGE_IMMUTABLE;   // 変更不可
	desc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	desc.CPUAccessFlags = 0;
	desc.MiscFlags = 0;
	desc.StructureByteStride = 0;
	...
	HRESULT hr = device->CreateBuffer(&desc, &subresourceData, rectangleVertexBuffer.GetAddressOf());
	_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
}